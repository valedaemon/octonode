// Generated by CoffeeScript 1.9.3
(function() {
  var Team,
    slice = [].slice;

  Team = (function() {
    function Team(id, client) {
      this.id = id;
      this.client = client;
    }

    Team.prototype.info = function(cb) {
      return this.client.get("/api/v3/teams/" + this.id, function(err, s, b, h) {
        if (err) {
          return cb(err);
        }
        if (s !== 200) {
          return cb(new Error('Team info error'));
        } else {
          return cb(null, b, h);
        }
      });
    };

    Team.prototype.update = function(info, cb) {
      return this.client.patch("/api/v3/teams/" + this.id, info, function(err, s, b, h) {
        if (err) {
          return cb(err);
        }
        if (s !== 200) {
          return cb(new Error('Team update error'));
        } else {
          return cb(null, b, h);
        }
      });
    };

    Team.prototype.members = function() {
      var cb, i, params, ref;
      params = 2 <= arguments.length ? slice.call(arguments, 0, i = arguments.length - 1) : (i = 0, []), cb = arguments[i++];
      return (ref = this.client).get.apply(ref, ["/api/v3/teams/" + this.id + "/members"].concat(slice.call(params), [function(err, s, b, h) {
        if (err) {
          return cb(err);
        }
        if (s !== 200) {
          return cb(new Error('Team members error'));
        } else {
          return cb(null, b, h);
        }
      }]));
    };

    Team.prototype.member = function(user, cb) {
      return this.client.get("/api/v3/teams/" + this.id + "/members/" + user, function(err, s, b, h) {
        if (err) {
          return cb(err);
        }
        return cb(null, s === 204, h);
      });
    };

    Team.prototype.addUser = function(user, cb) {
      return this.client.put("/api/v3/teams/" + this.id + "/members/" + user, null, function(err, s, b, h) {
        if (err) {
          return cb(err);
        }
        if (s !== 204) {
          return cb(new Error("Team addUser error"));
        } else {
          return cb(null, b, h);
        }
      });
    };

    Team.prototype.removeUser = function(user, cb) {
      return this.client.del("/api/v3/teams/" + this.id + "/members/" + user, null, function(err, s, b, h) {
        if (err) {
          return cb(err);
        }
        if (s !== 204) {
          return cb(new Error("Team removeUser error"));
        } else {
          return cb(null, b, h);
        }
      });
    };

    Team.prototype.membership = function(user, cb) {
      return this.client.get("/api/v3/teams/" + this.id + "/memberships/" + user, function(err, s, b, h) {
        if (err) {
          return cb(err);
        }
        return cb(null, s === 204, h);
      });
    };

    Team.prototype.addMembership = function(user, cb) {
      return this.client.put("/api/v3/teams/" + this.id + "/memberships/" + user, null, function(err, s, b, h) {
        if (err) {
          return cb(err);
        }
        if (s !== 200) {
          return cb(new Error("Team membership error"));
        } else {
          return cb(null, b, h);
        }
      });
    };

    Team.prototype.removeMembership = function(user, cb) {
      return this.client.del("/api/v3/teams/" + this.id + "/memberships/" + user, null, function(err, s, b, h) {
        if (err) {
          return cb(err);
        }
        if (s !== 204) {
          return cb(new Error("Team removeMembership error"));
        } else {
          return cb(null, b, h);
        }
      });
    };

    Team.prototype.repos = function() {
      var cb, i, params, ref;
      params = 2 <= arguments.length ? slice.call(arguments, 0, i = arguments.length - 1) : (i = 0, []), cb = arguments[i++];
      return (ref = this.client).get.apply(ref, ["/api/v3/teams/" + this.id + "/repos"].concat(slice.call(params), [function(err, s, b, h) {
        if (err) {
          return cb(err);
        }
        if (s !== 200) {
          return cb(new Error("Team repos error"));
        } else {
          return cb(null, b, h);
        }
      }]));
    };

    Team.prototype.destroy = function(cb) {
      return this.client.del("/api/v3/teams/" + this.id, {}, function(err, s, b, h) {
        if (err) {
          return cb(err);
        }
        if (s !== 204) {
          return cb(new Error("Team destroy error"));
        } else {
          return cb(null, b, h);
        }
      });
    };

    return Team;

  })();

  module.exports = Team;

}).call(this);
