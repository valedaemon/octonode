...
(function() {
  ...
  auth = module.exports = {
    ...
    config: function(options) {
      ...
      if (options.baseUrl) {
        options.oauth_endpoint = options.baseUrl + "/api/v3/authorizations/"
      } else {
        options.oauth_endpoint = "https://api.github.com/authorizations/"
      }
    ...
    revoke: function(id, callback) {
      ...
      if (this.mode === this.modes.cli) {
        options = {
          url: baseUrl.parse(this.options.oauth_endpoint + id),
          method: 'DELETE',
          headers: {
    ...
    login: function(scopes, callback) {
      ...
      if (this.mode === this.modes.cli) {
        ...
        options = {
          url: baseUrl.parse(this.options.oauth_endpoint),
          method: 'POST',
          body: scopes,
          headers: {
        ...
        if (scopes instanceof Array) {
          uri = this.options.baseUrl + '/login/oauth/authorize';
          ...
          return request({
            url: this.options.baseUrl + '/login/oauth/access_token',
            method: 'POST',
            body: qs.stringify({
              code: scopes,
